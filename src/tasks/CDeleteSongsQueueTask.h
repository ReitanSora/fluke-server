...
/**
 * @class CDeleteSongsQueueTask
 * @brief Task that deletes songs from the queue.
 *
 * This class implements the IAIMPTask interface and is used to query AIMP for
 * deleting a given song from the queue.
 *
 ...
 */
class CDeleteSongsQueueTask : public IAIMPTask
{
private:
    /**
...
public:
     ...
     */
    CDeleteSongsQueueTask(MyPlugin* plugin, const std::string& _playlistId, const std::vector<int>& _songsIndexes);
