...
/**
 * @class CAddSongsTask
 * @brief Task that adds songs to a specific playlist.
 *
 * This class implements the IAIMPTask interface and is used to query AIMP for
 * adding songs to a given playlist.
 *
 ...
 */
class CAddSongsTask : public IAIMPTask
{
private:
    /**
     ...
     */
    std::string _targetPlaylistId;
    std::string _sourcePlaylistId;

    std::vector<int> _songsIndexes;

    bool _playlistsFound = false;
...
public:
     ...
     */
    CAddSongsTask(MyPlugin* plugin, const std::string& _targetPlaylistId, const std::string& _sourcePlaylistId, const std::vector<int>& _songsIndexes);
    ...
    bool HasFoundPlaylists() const { return _playlistsFound; }
};