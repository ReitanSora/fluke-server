...
/**
 * @class CCreatePlaylistTask
 * @brief Task that creates a new playlist.
 *
 * This class implements the IAIMPTask interface and is used to query AIMP for
 * creating a new playlist with a specified name.
 *
 ...
 */
class CCreatePlaylistTask : public IAIMPTask
{
private:
    /**
...
public:
     ...
     */
    CCreatePlaylistTask(MyPlugin* plugin, const std::string& _playlistName);
	...
	std::string GetCreatedPlaylistId() const { return _createdPlaylistId; }
