...
/**
 * @class AddSongsQueueTask
 * @brief Task that adds songs to the queue.
 *
 * This class implements the IAIMPTask interface and is used to query AIMP for
 * adding a given song to the queue.
 *
 ...
 */
class CAddSongsQueueTask : public IAIMPTask
{
private:
    /**
...
public:
     ...
     */
    CAddSongsQueueTask(MyPlugin* plugin, const std::string& _playlistId, const std::vector<int>& _songsIndexes, const bool& _insertAtBeginning);
